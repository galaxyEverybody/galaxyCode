    ...
    if ($GLOBALS['db']->getOne($sql) || $parent_id == 0)
    {
        ...
        foreach ($res AS $row)
        {
			...
			foreach ($brands AS $key => $val)
			{
				...
				$brands[$key]['url'] = build_uri('brand', array('cid' => $cat_id, 'bid' => $val['brand_id']), $row['cat_name']);
			}
...
function get_recommend_goods($type = '', $cats = '')
{
    ...
    if (empty($type_goods[$type]))
    {
        ...
        $sql = 'SELECT g.goods_id, g.goods_name, g.ensure_style, g.good_status, g.add_time, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price, g.surplus_price,g.goods_weight,g.goods_number, ' .
        ...
        foreach ($result AS $idx => $row)
        {
            ...
            $goods[$idx]['ensure_style']        = $row['ensure_style'];
 ...
 * @return  array
 */
function get_category_recommend_goods($type = '', $dwt= '',$cats = '', $brand = 0, $min =0,  $max = 0, $ext='')
{
	...
	if(!empty($dwt))
	{
   		...
   		$num = get_library_number($type2lib[$type],$dwt);
   ...
   //开始获取子分类
	...
	if($subres)
	{
		foreach ($subres as $sidx => $subrow)
		{
			...
			$children[$sidx]['keywords']=$subrow['keywords'];
			$children[$sidx]['des']=$subrow['cat_desc'];
...
function get_goods_fittings($goods_list = array())
{
    ...
    $sql = 'SELECT gg.parent_id, ggg.goods_name AS parent_name, gg.group_id, gg.goods_id, gg.goods_price, g.goods_name, g.goods_thumb, g.goods_img, g.shop_price AS org_price,  g.promote_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price ".
