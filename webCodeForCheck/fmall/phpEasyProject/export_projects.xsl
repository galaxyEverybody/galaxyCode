...
-->
	...
	<xsl:template match="project" mode="html">
		...
		<table style="width:100%;padding-bottom:16px" cellspacing="0" cellpadding="8">
			<xsl:apply-templates mode="html" />
		</table>
	</xsl:template>
	...
	<xsl:template match="username" mode="html">
		<td>
			<xsl:apply-templates mode="html" />
		</td>
	...
	<xsl:template match="prpossum" mode="html">
		<tr>
	...
	<xsl:template match="projectsum" mode="html">
			<tr>
			<td colspan="2"><hr /><h2><xsl:value-of select="@name" /></h2></td>
			<td style="text-align:right;"><hr /><h2>
				<xsl:apply-templates mode="html" />
				<xsl:text>&#160;</xsl:text>
				<xsl:value-of select="@suffix" /></h2></td>
		</tr>
	</xsl:template>
